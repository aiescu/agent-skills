...
- **Do NOT add this skill to a persona's `skills:` frontmatter.** A persona that follows Step 3 would spawn another persona — the orchestration anti-pattern explicitly forbidden by `references/shared/orchestration-patterns.md` ("personas do not invoke other personas").
...
The upstream role-based reviewer profiles are bundled in `references/reviewers/` for the roster and per-domain match. These are reference profiles, not automatically registered agents; invoke them through the available runtime with isolated context, or use the generic-subagent fallback below.
...
- **Repo orchestration rules** (`references/shared/orchestration-patterns.md`): this skill orchestrates from the main session. A persona calling another persona is anti-pattern B — see Loading Constraints above.
...
Distributed by Aiescu. Original author: Addy Osmani. Source: [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills/tree/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/skills/doubt-driven-development).
